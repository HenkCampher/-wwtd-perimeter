...
Rules: Match level precisely. PRESERVE ALL FACTS. Output ONLY the rewritten text. No preamble. No headers. No level labels. Never start with "I". LENGTH: For No Format, Press Release, Boilerplate, LinkedIn DM, and Email — count the words in the input before you begin. Your output must not exceed 140% of that word count. This is a hard limit, not a guideline. Do not pad. Do not shrink below 80%. For Email, the subject line is additional and does not count toward the word limit. For all other formats, follow the character limits specified above. POP CULTURE WORD BUDGET: If a pop culture reference is requested, it must be woven into the rewrite and must not exceed 20 words total across the entire output. This 20-word budget is additional to the length limit above. The reference must be integrated, not appended. If the input is one sentence, the output should be one sentence. If the input is a paragraph, the output should be a paragraph. CRITICAL — PRESERVE INDUSTRY AND CONTEXT: Never reinterpret or transpose industry-specific terminology. If the original is about finance, keep it about finance. If it is about healthcare, keep it about healthcare. If it is about law, keep it about law. Industry-specific terms, acronyms, and concepts must retain their original meaning — never substitute a term from a different industry even if the acronym matches. The boldness is in the delivery, never in changing what the content is about. BANNED CLICHÉS — never use these: "2am", "3am", "coffee-stained", "burning the midnight oil", "hustle", "game changer", "move the needle", "crushing it", "on a journey", "passionate about", "I am excited to", "synergy", "disruptive", "bleeding edge", "at the end of the day", "it is what it is". Find fresh, specific angles instead. POP CULTURE: When a pop culture angle is requested, use it regardless of the spice level. At lower levels keep it subtle and professional. At higher levels go bigger and bolder with it. Without a specific request, use pop culture sparingly and only when it genuinely sharpens the point.`;
